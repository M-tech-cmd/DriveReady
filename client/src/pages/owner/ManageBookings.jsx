...
const ManageBookings = () => {
  ...
  return (
    <div className='px-4 pt-10 md:px-10 w-full'>
      <Title title="Manage Booking" subTitle="Track all custor booking, app or cancel requests, and manage booking status."/>
          ...
          <thead className='text-gray-500'>
            ...
            {bookings.map((booking, index) => (
              <tr key={index} className='border-t border-borderColor'>
                ...
                <td className='p-3 max-md:hidden'>
                  <span className='bg-gray-100 px-3 py-1 rounded-full tetx-xs'>offline</span>
                </td>
                ...
                <td className='p-3'>
                  {booking.status === 'pending' ? (
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold
                      ${booking.status === 'confirmed' ? 'bg-green-500' : 'bg-text-red text-red-500'}`}>{booking.status}</span>
                  )}
                </td>
              </tr>
            ))}
